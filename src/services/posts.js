...
export async function getPosts(page = 1, limit = 10) {
    try {
         const API = await fetch(`http://localhost:3000/posts?page=${page}&limit=${limit}`, {
       headers: {
...
export async function getPostsByNameOrDescription(seacrh , page = 1, limit = 10) {
  try {
      if(!seacrh || String(seacrh).length ==0){
        return false
      }
      const API = await fetch(`http://localhost:3000/posts/${seacrh}?page=${page}&limit=${limit}`, {
       headers: {
            "Content-Type": "application/json",
            "authorization" : localStorage.getItem("token")
        },
...
export async function getPostByUserId(userid , page = 1, limit = 8) {
    try {
        const API = await fetch(`http://localhost:3000/postbyuser/${userid}?page=${page}&limit=${limit}`, {
          headers: {
            "Content-Type": "application/json",
            authorization: localStorage.getItem("token"),
          },
